...
import { Pencil, Calendar, Clock, User, Link2, FileText, AlertCircle, FolderOpen, Image } from 'lucide-react'
...
const labels = {
  zh: {
    ...
    completionPlaceholder: '留空则按工期自动计算',
    ownerLabel: '责任人',
    linkLabel: '项目链接',
...
export default function EditProjectDialog({ project, status = 'in_progress' }: { project: Project; status?: ProjectStatus }) {
  ...
  const [form, setForm] = useState(() => ({
    ...
    startDate: format(new Date(project.plannedStartDate), 'yyyy-MM-dd'),
    duration: String(project.duration),
    description: project.description ?? '',
    owner: project.owner ?? '',
    link: project.link ?? '',
    image: project.image ?? '',
    completionTime: project.completionTime ? format(new Date(project.completionTime), 'yyyy-MM-dd') : '',
  }))
  ...
  function validate(): boolean {
    ...
    if (!form.name.trim()) newErrors.name = lang === 'zh' ? '请填写项目简称' : 'Short name is required'
    ...
    if (!form.completionTime) newErrors.completionTime = lang === 'zh' ? '请填写完成日期' : 'Completion date is required'
    if (!form.owner.trim()) newErrors.owner = lang === 'zh' ? '请填写责任人' : 'Owner is required'
    setErrors(newErrors)
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder={lang === 'zh' ? '输入项目简称' : 'Enter short name'}
              />
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.fullName}
                ...
                className="border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:text-gray-100 transition-all"
                placeholder={lang === 'zh' ? '输入项目全称' : 'Enter full name'}
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.owner}
                ...
                className={`${errors.owner ? 'border-red-300 bg-red-50 dark:bg-red-900/20 focus:border-red-400 focus:ring-red-100' : 'border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 focus:border-blue-400 focus:ring-blue-100'} dark:text-gray-100 focus:ring-2 transition-all`}
                placeholder={lang === 'zh' ? '输入责任人' : 'Enter owner'}
              />
          ...
          <div className="space-y-1.5">
            ...
            <Textarea value={form.description}
              ...
              className="border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:text-gray-100 transition-all resize-none"
              rows={3}
              placeholder={lang === 'zh' ? '输入项目描述...' : 'Enter description...'}
            />
          </div>
          <div className="flex justify-end gap-3 pt-3 border-t border-gray-100 dark:border-gray-700">
            <Button type="button" variant="outline" onClick={() => setOpen(false)} className="border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-700 dark:hover:text-gray-100">{t.cancel}</Button>
            <Button type="submit" className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 shadow-md shadow-blue-200 dark:shadow-blue-900/50 font-medium">{t.save}</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
